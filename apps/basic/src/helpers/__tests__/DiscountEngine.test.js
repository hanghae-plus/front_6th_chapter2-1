...
describe('DiscountEngine', () => {
  ...
  describe('combineDiscounts', () => {
    ...
    it('단일 할인은 그대로 반환', () => {
      const discounts = [
        { type: DISCOUNT_TYPES.INDIVIDUAL, rate: 0.15, description: '개별할인' }
      ];
  ...
  describe('_getAllAvailableDiscounts', () => {
    it('모든 적용 가능한 할인을 찾아서 반환', () => {
      const discounts = DiscountEngine._getAllAvailableDiscounts(
        mockCart,
        mockContext
      );

      // 개별 할인 (상품1: 12개)
      const individualDiscount = discounts.find(
      ...
      // 번개세일 할인 (상품2, 상품3)
      const flashDiscount = discounts.find(
        d => d.type === DISCOUNT_TYPES.FLASH
      );
      ...
      // 조합할인 (상품3)
      const comboDiscount = discounts.find(
        d => d.type === DISCOUNT_TYPES.COMBO
      );
  ...
  describe('applyDiscountPolicies', () => {
    it('최적 할인 조합을 적용하여 최종 결과 반환', () => {
      const result = DiscountEngine.applyDiscountPolicies(
        mockCart,
        mockContext
      );
      ...
      expect(result.finalAmount).toBeLessThan(
        result.appliedDiscounts.length ? 240000 : 240000
      ); // 원래 총액: 12*10000 + 8*15000 + 5*20000 = 340000
  ...
  describe('findBestDiscount', () => {
    it('최대 절약 금액을 제공하는 할인 조합 선택', () => {
      ...
      const bestCombination =
        DiscountEngine.findBestDiscount(availableDiscounts);

      // 최대 할인율을 가진 조합이 선택되어야 함
      expect(bestCombination.length).toBeGreaterThan(0);
      const totalRate = bestCombination.reduce(
