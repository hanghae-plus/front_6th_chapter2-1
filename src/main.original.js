...
function main(){
  ...
  totalAmt = 0; itemCnt = 0; lastSel = null;
  ...
  prodList=[
    ...
    {  id  :  p3_id  ,  name  :  "거북목 탈출 모니터암"  ,  val  :  30000  , originalVal: 30000, q  :  20, onSale: false, suggestSale: false  }  ,
    {id:p4,name:"에러 방지 노트북 파우치",val:15000,originalVal:15000,q:0,onSale:false,suggestSale:false},
    { id: productFive, name: `코딩할 때 듣는 Lo-Fi 스피커`, val: 25000, originalVal: 25000, q: 10, onSale: false, suggestSale: false }
  ]

  var root=document.getElementById('app')

  header = document.createElement( 'div' ); header.className= 'mb-8'
  header.innerHTML = `
  ...
  gridContainer=document.createElement('div'); gridContainer.className='grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 flex-1 overflow-hidden';

  leftColumn = document.createElement("div"); leftColumn['className']='bg-white border border-gray-200 p-8 overflow-y-auto'

  selectorContainer=document.createElement('div'); selectorContainer.className='mb-6 pb-6 border-b border-gray-200';

  sel=document.createElement('select'); sel.id='product-select'; sel.className='w-full p-3 border border-gray-300 rounded-lg text-base mb-3';

  addBtn=document.createElement('button'); addBtn.id='add-to-cart'; addBtn.innerHTML='Add to Cart'; addBtn.className='w-full py-3 bg-black text-white text-sm font-medium uppercase tracking-wider hover:bg-gray-800 transition-all';

  stockInfo=document.createElement('div'); stockInfo.id='stock-status'; stockInfo.className='text-xs text-red-500 mt-3 whitespace-pre-line';

  selectorContainer.appendChild(sel); selectorContainer.appendChild(addBtn); selectorContainer.appendChild(stockInfo);

  cartDisp=document.createElement('div'); cartDisp.id='cart-items';

  leftColumn.appendChild(selectorContainer); leftColumn.appendChild(cartDisp);

  rightColumn=document.createElement('div'); rightColumn.className='bg-black text-white p-8 flex flex-col';
  ...
  gridContainer.appendChild(leftColumn); gridContainer.appendChild(rightColumn);

  manualOverlay.appendChild(manualColumn);

  root.appendChild(header); root.appendChild(gridContainer); root.appendChild(manualToggle); root.appendChild(manualOverlay);
  ...
  setTimeout(function () {
    setInterval(function () {
      ...
      if(lastSel) {
        var suggest=prodList.find(function (item) { return item.id !== lastSel && item.q > 0 && !item.suggestSale; });
        if(suggest) {
          alert('💝 ' + suggest.name + '은(는) 어떠세요? 지금 구매하시면 5% 추가 할인!');
          // 추천할인: 현재가의 95% (5% 추가 할인)
          // 번개세일과 중복 가능
...
function calcCart() { 
  ...
  totalAmt=0; itemCnt=0; cartItems=cartDisp.children; subTot=0; itemDiscounts = [];
  ...
  for (let i=0; i < cartItems.length; i++) {
    (function () {
      ...
      q=parseInt(qtyElem.textContent); itemTot=curItem.val * q; disc=0;

      itemCnt += q; subTot += itemTot;

      var itemDiv = cartItems[i]; var priceElems = itemDiv.querySelectorAll('.text-lg, .text-xs');
      ...
      if(q >= 10) {
        if(curItem.id === PRODUCT_ONE) disc=0.1; else if(curItem.id === PRODUCT_TWO) disc=0.15; else if(curItem.id === p3_id) disc=0.2; else if(curItem.id === p4) disc=0.05; else if(curItem.id === productFive) disc=0.25;
        if(disc > 0) {
  ...
  let discRate=0; var originalTotal = subTot;
  
  // Step 1: 개별 상품 할인 또는 대량 구매 할인 적용
  if(itemCnt >= 30) {
    // 30개 이상: 대량 구매 할인 25% (개별 할인 무시)
  ...
  // 포인트 계산은 renderBonusPts에서 처리되므로 여기서는 삭제
...
function updateCartPrices() {
  ...
  for(var i = 0; i < cartItems.length; i++) {
    ...
    var product = prodList.find(function(p) { return p.id === itemId; });
    if(product) {
...
addBtn.addEventListener( "click" , function( ){
  ...
  if(!selItem || !prodList.some(p => p.id === selItem)) {
    return;
  }
  ...
  if( itemToAdd&&itemToAdd.q>0 ){
    ...
    if(item){
      ...
      if( newQty<=itemToAdd.q+parseInt(qtyElem.textContent) ){
        qtyElem.textContent=newQty; itemToAdd['q']--
      ...
      `;
      cartDisp.appendChild(newItem); itemToAdd.q--;
    }
  ...
  if( tgt.classList.contains( 'quantity-change' )||tgt.classList.contains( "remove-item" ) ){
    ...
    if(tgt.classList.contains('quantity-change')) {
      ...
      if(newQty > 0 && newQty <= prod.q + currentQty) {
        qtyElem.textContent = newQty; prod.q -= qtyChange;
      } else if(newQty <= 0) {
        prod.q += currentQty; itemElem.remove();
      } else {
        alert('재고가 부족합니다.');
      }
