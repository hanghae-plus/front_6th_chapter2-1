...
export function calculateCartTotals(cartItems, productList) {
  ...
  return { totalAmt, itemCnt, subtotal, itemDiscounts };
}

export function applyBulkAndSpecialDiscounts(totalAmt, itemCnt, subtotal) {
  let discRate = 0;
  ...
  if (itemCnt >= QUANTITY_THRESHOLDS.BULK_PURCHASE) {
    ...
    discRate = (subtotal - totalAmt) / subtotal;
  }
