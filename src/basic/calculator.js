import { PRODUCT_ID, DISCOUNT, POINTS } from './constants.js';
...
export function calculateDiscounts(cart, products) {
  let totalDiscount = 0;
  const itemDiscounts = [];
  ...
  if (totalQuantity >= DISCOUNT.BULK_DISCOUNT_THRESHOLD) {
    ...
    cart.forEach((item) => {
      ...
      if (item.quantity >= 10) {
        switch (product.id) {
          case PRODUCT_ID.P1:
            discountRate = DISCOUNT.KEYBOARD_DISCOUNT_RATE;
            break;
          case PRODUCT_ID.P2:
            discountRate = DISCOUNT.MOUSE_DISCOUNT_RATE;
            break;
          case PRODUCT_ID.P3:
            discountRate = DISCOUNT.MONITOR_ARM_DISCOUNT_RATE;
            break;
          case PRODUCT_ID.P5:
            discountRate = DISCOUNT.SPEAKER_DISCOUNT_RATE;
            break;
          default:
            break;
        }
...
export function calculatePoints(cart, total, products) {
  ...
  // 세트 구매 보너스
  const hasProduct = (productId) => cart.some((item) => item.id === productId);
