...
function main() {
  ...
  cartDisp = $('<div id="cart-items"></div>');
...
function onUpdateSelectOptions() {
  ...
  function getOptionTextContent(item) {
    if (item.q === 0) {
      return `${item.name} - ${item.val}원 (품절)`;
    } else if (item.onSale && item.suggestSale) {
      return `⚡💝${item.name} - ${item.originalVal}원 → ${item.val}원 (25% SUPER SALE!)`;
    } else if (item.onSale) {
      return `⚡${item.name} - ${item.originalVal}원 → ${item.val}원 (20% SALE!)`;
    } else if (item.suggestSale) {
      return `💝${item.name} - ${item.originalVal}원 → ${item.val}원 (5% 추천할인!)`;
    } else {
  ...
  function getOptionDisabled(item) {
    return item.q === 0;
  }

  function renderOption(item) {
    const opt = $(`<option value="${item.id}"></option>`);
    opt.textContent = getOptionTextContent(item);
    opt.className = getOptionClassName(item);
    if (getOptionDisabled(item)) opt.disabled = true;
    return opt;
  }

  //
  const totalStock = prodList.reduce((acc, curr) => acc + curr.q, 0);
