export const createProductOption = ({ item }) => {
  ...
  if (item.q === 0) {
    ...
    if (item.onSale && item.suggestSale) {
      // 세일 추천 상품
      productOption.textContent = '⚡💝' + item.name + ' - ' + item.originalVal + '원 → ' + item.val + '원 (25% SUPER SALE!)';
      productOption.className = 'text-purple-600 font-bold';
    } else if (item.onSale) {
      // 세일 상품
      productOption.textContent = '⚡' + item.name + ' - ' + item.originalVal + '원 → ' + item.val + '원 (20% SALE!)';
      productOption.className = 'text-red-500 font-bold';
    } else if (item.suggestSale) {
      // 추천 상품
      productOption.textContent = '💝' + item.name + ' - ' + item.originalVal + '원 → ' + item.val + '원 (5% 추천할인!)';
      productOption.className = 'text-blue-500 font-bold';
    } else {
      // 일반 상품
      productOption.textContent = item.name + ' - ' + item.val + '원' + discountText;
    }
  }
