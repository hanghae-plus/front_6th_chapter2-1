...
export const HelpModal: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* 모달 오버레이 */}
      <div 
        className={`fixed inset-0 bg-black/50 z-40 transition-opacity duration-300 ${isOpen ? '' : 'hidden'}`}
        onClick={closeModal}
      >
        {/* 모달 패널 */}
        <div 
          className={`fixed right-0 top-0 h-full w-80 bg-white shadow-2xl p-6 overflow-y-auto z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
