...
export type AppSummaryState = Pick<
  AppState,
  'totalProductCount' | 'totalBeforeDiscount' | 'totalAfterDiscount' | 'totalDiscountedRate' | 'discountedProductList'
>;

export interface CartBonusState {
  state: CartSummaryState;
  appState: AppSummaryState;
}

// 포인트 계산
export const calculateBonusPoint = ({ state, appState }: CartBonusState) => {
