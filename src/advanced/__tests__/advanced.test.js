...
vi.mock('@/advanced/api/getProducts', () => ({
  default: vi.fn().mockResolvedValue(mockProducts),
}));
...
describe('advanced 테스트', () => {
  ...
  describe.each([{ type: 'origin', loadFile: () => import('../../main.original.js') }])(
    '$type 장바구니 상세 기능 테스트',
    ({ loadFile }) => {
      ...
      beforeEach(async () => {
        ...
        // advanced 앱의 경우 React 렌더링을 기다림
        if (loadFile.toString().includes('main.advanced')) {
          // React 렌더링 완료를 기다림
