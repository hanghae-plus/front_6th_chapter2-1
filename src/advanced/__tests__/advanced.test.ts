...
describe('advanced 테스트', () => {
  // 공통 헬퍼 함수
  const addItemsToCart = (sel: HTMLSelectElement, addBtn: HTMLButtonElement, productId: string, count: number): void => {
    sel.value = productId
  ...
  const expectProductInfo = (option: HTMLOptionElement, product: Product): void => {
  ...
  const getCartItemQuantity = (cartDisp: HTMLElement, productId: string): number => {
  ...
  describe.each([
    ...
    let cartDisp: HTMLElement
    let sum: HTMLElement
    let stockInfo: HTMLElement
    let itemCount: HTMLElement
    let loyaltyPoints: HTMLElement
    let discountInfo: HTMLElement
    ...
    beforeEach(async () => {
      ...
      // React 컴포넌트인 경우와 JS 모듈인 경우 구분
      if (loadFile.toString().includes('components/ShoppingCart')) {
        // React 컴포넌트 렌더링
        const { default: ShoppingCart } = await loadFile()
      ...
      addBtn = document.getElementById('add-to-cart') as HTMLButtonElement
      ...
      describe('2.1 상품 목록', () => {
        it('5개 상품이 올바른 정보로 표시되어야 함', () => {
          ...
          expectedProducts.forEach((product, index) => {
            expectProductInfo(sel.options[index] as HTMLOptionElement, product)
          })
        })
      })
      ...
      describe('3.3 특별 할인', () => {
        describe('3.3.1 화요일 할인', () => {
          it('화요일에 10% 추가 할인 적용', () => {
            ...
            // 화요일 특별 할인 배너 표시
            const tuesdayBanner = document.getElementById('tuesday-special') as HTMLElement
    ...
    describe('4. 포인트 적립 시스템', () => {
      ...
      describe('4.3 포인트 표시', () => {
        it('포인트 적립 내역 상세 표시', () => {
          ...
          const pointsText: string = loyaltyPoints.textContent || ''
    ...
    describe('5. UI/UX 요구사항', () => {
      describe('5.1 레이아웃', () => {
        it('필수 레이아웃 요소가 존재해야 함', () => {
          ...
          expect(h1Element.textContent).toContain('🛒 Hanghae Online Store')
          
          const titleElement = document.querySelector('.text-5xl') as HTMLElement
          expect(titleElement.textContent).toContain('Shopping Cart')
      ...
      describe('5.3 장바구니 영역', () => {
        it('장바구니 아이템 카드 형식 확인', () => {
          ...
          const cartItem = cartDisp.querySelector('#p1') as HTMLElement

          // 상품 이미지
          expect(cartItem.querySelector('.bg-gradient-black')).toBeTruthy()

          // 상품명
          const productName = cartItem.querySelector('h3') as HTMLElement
          expect(productName.textContent).toContain('버그 없애는 키보드')
        ...
        it('첫 번째 상품은 상단 여백 없음', () => {
          ...
          const firstItem = cartDisp.firstElementChild as HTMLElement
        ...
        it('마지막 상품은 하단 테두리 없음', () => {
          ...
          const lastItem = cartDisp.lastElementChild as HTMLElement
    ...
    describe('6. 기능 요구사항', () => {
      describe('6.1 상품 추가', () => {
        ...
        it('이미 있는 상품은 수량 증가', () => {
          ...
          expect(cartDisp.children.length).toBe(1)
          const qtyElement = cartDisp.querySelector('.quantity-number') as HTMLElement
      ...
      describe('6.2 수량 변경', () => {
        it('+/- 버튼으로 수량 조절', async () => {
          ...
          const qtyAfterIncrease = cartDisp.querySelector('.quantity-number') as HTMLElement
          expect(qtyAfterIncrease.textContent).toBe('2')

          // 감소
          await userEvent.click(decreaseBtn)
          const qtyAfterDecrease = cartDisp.querySelector('.quantity-number') as HTMLElement
          expect(qtyAfterDecrease.textContent).toBe('1')
        })
