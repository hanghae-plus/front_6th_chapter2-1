...
function ModalHeader({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex justify-between items-center mb-4">
      ...
      <button className="text-gray-500 hover:text-black manual-close-btn" onClick={onClose}>
...
function ToggleButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="fixed top-4 right-4 bg-black text-white p-3 rounded-full hover:bg-gray-900 transition-colors z-50">
...
// 모달 오버레이 컴포넌트
function ModalOverlay({ isOpen, onClose, children }: { isOpen: boolean; onClose: () => void; children: React.ReactNode }) {
...
// 모달 컨텐츠 컴포넌트
function ModalContent({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  return (
  ...
  return (
    <>
      ...
      <ModalOverlay isOpen={isOpen} onClose={handleClose}>
        <ModalContent isOpen={isOpen} onClose={handleClose} />
      </ModalOverlay>
    </>
  );
}
