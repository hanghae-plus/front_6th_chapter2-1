...
const HelpModal = ({ onToggle }: HelpModalProps) => {
  ...
  return (
    <>
      <button
        ...
        className='help-toggle fixed top-4 right-4 bg-black text-white p-3 rounded-full hover:bg-gray-900 transition-colors z-50'
      >
        <svg
          className='w-5 h-5'
          fill='none'
      ...
      {/* Modal Overlay */}
      <div
        className={`help-overlay fixed inset-0 bg-black/50 z-40 transition-opacity duration-300 ${
          isOpen ? '' : 'hidden'
        }`}
      ...
      >
        ...
        >
          <button
            onClick={handleClose}
            className='help-close absolute top-4 right-4 text-gray-500 hover:text-black'
          >
            <svg
              className='w-6 h-6'
              fill='none'
